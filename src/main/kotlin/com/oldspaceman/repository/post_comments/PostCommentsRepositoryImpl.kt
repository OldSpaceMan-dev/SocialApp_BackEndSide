...
import com.oldspaceman.dao.post_comments.PostCommentRow
...
class PostCommentsRepositoryImpl(
        ...
        return if (postCommentRow == null) {
            Response.Error(
                ...
                data = CommentResponse(
                    success = false,
                    message = "Could not insert comment into the DataBase"
                )
            )
        } else{
            //increment number of comment count in the post table
    ...
    override suspend fun removeComment(params: RemoveCommentParams): Response<CommentResponse> {
        ...
        return if (commentRow == null) {
            ...
            val postOwnerId = postDao.getPost(postId = params.postId)!!.userId  //!! - should not null -- take userID - create post
